...
public abstract class BaseClassAdapter extends MyBaseAdapter implements View.OnClickListener {
    ...
    public BaseClassAdapter(ArrayList<?> list, BaseActivity context, int columNum) {
        ...
        int viewSpace = context.getResources().getDimensionPixelOffset(R.dimen.view_space);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            holder = (ParentViewHolder) convertView.getTag(R.id.tag_viewholder);
        }
        initContentView(holder.contentView, getItemList(position));
    ...
    // 填充ChildItem View的内容
    protected abstract void bindItemData(Object obj, ItemViewHolder holder);
    ...
    // 初始化显示childItem的容器
    protected synchronized void initContentView(LinearLayout contentView, ArrayList<?> list) {
        ...
        if(columNum == 1){ // 每个item一行
            for (int i = 0; i < size; i++) {
                ...
                View v1 = getItemView(list.get(i));
                ...
                if (p == size - 1) { // 如果换行的第一个item刚好是最后一个, 就不需要创建一个"行"的容器
                    ...
                    for (int j = 1; j < columNum; j++) {
                        ...
                        if (index < size) { // 还没有超过数组的总数, 继续添加
                            View vj = getItemView(list.get(index));
    ...
    private synchronized View getItemView(Object obj) {
        View v;

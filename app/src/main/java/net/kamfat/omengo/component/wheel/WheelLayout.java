package net.kamfat.omengo.component.wheel;
...
public class WheelLayout extends LinearLayout {
	 ...
	 * 初始化滚动的控件
	 */
	private void initDateWheelView(int year, int month, int day, int hour,
	...
	public String getTime() {
		...
		if (yearView != null) {
			sb.append((yearView.getCurrentItem() + START_YEAR)).append("/")
					.append((monthView.getCurrentItem() + 1)).append("/")
					.append((dayView.getCurrentItem() + 1));
		}
		if (hourView != null) {
			if (sb.length() > 0) {
				sb.append(" ");
